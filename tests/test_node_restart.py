...
def test_restart_single_node():
    ...
    """
    # Setup
    config_mgr = ConfigurationManager(ClusterConfig(num_shards=1, replicas_per_shard=1), PortManager())
    valkey_binary = config_mgr.setup_valkey_from_source()

    config = ClusterConfig(
        num_shards=1, replicas_per_shard=1, base_port=7600, valkey_binary=valkey_binary, enable_cleanup=True
    )
...
def test_restart_primary_in_multi_shard_cluster():
    ...
    """
    # Setup
    config_mgr = ConfigurationManager(ClusterConfig(num_shards=3, replicas_per_shard=1), PortManager())
    valkey_binary = config_mgr.setup_valkey_from_source()

    config = ClusterConfig(
        num_shards=3, replicas_per_shard=1, base_port=7700, valkey_binary=valkey_binary, enable_cleanup=True
    )
...
def test_restart_multiple_nodes_sequentially():
    ...
    config = ClusterConfig(
        num_shards=2, replicas_per_shard=1, base_port=7800, valkey_binary=valkey_binary, enable_cleanup=True
    )
