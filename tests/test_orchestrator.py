...
def test_setup_valkey_from_source_uses_configured_binary(mock_isfile, mock_access):
    ...
    config = ClusterConfig(num_shards=3, replicas_per_shard=1, valkey_binary="/tmp/valkey/src/valkey-server")
...
def test_setup_valkey_from_source_fails_when_configured_binary_missing(mock_isfile):
    ...
    config = ClusterConfig(num_shards=3, replicas_per_shard=1, valkey_binary="/tmp/missing-valkey-server")
    config_mgr = ConfigurationManager(config, PortManager())
...
def test_full_cluster_creation():
    ...
    config = ClusterConfig(num_shards=2, replicas_per_shard=1, base_port=7100, valkey_binary=valkey_binary)
...
def test_full_cluster_creation_large():
    ...
    config = ClusterConfig(num_shards=3, replicas_per_shard=2, base_port=7200, valkey_binary=valkey_binary)
