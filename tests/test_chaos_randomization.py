...
def test_specific_nodes_not_randomized(chaos_coordinator):
    ...
    config = ChaosConfig(
        chaos_type=ChaosType.PROCESS_KILL,
        target_selection=TargetSelection(strategy="specific", specific_nodes=["node_0", "node_1"]),
        timing=ChaosTiming(delay_before_operation=1.0, delay_after_operation=1.0),
        coordination=ChaosCoordination(chaos_during_operation=True),
...
def test_timing_delay_randomization(mock_sleep, chaos_coordinator, mock_cluster_connection):
    ...
    """
    chaos_coordinator.chaos_engine.inject_process_chaos = Mock(
        return_value=Mock(
            success=True,
            chaos_id="test",
            chaos_type=ChaosType.PROCESS_KILL,
...
def test_chaos_randomization_across_operations(mock_sleep, mock_cluster_connection):
    ...
    def mock_inject_chaos(node, chaos_type, **kwargs):
        ...
        chaos_injections.append({"node_id": node.node_id, "node_role": node.role, "chaos_type": chaos_type})
    ...
    assert len(chaos_injections) <= 3, (
        f"Expected at most 3 chaos injections (shard-safety limit), got {len(chaos_injections)}"
    )
...
def test_chaos_without_randomization_is_consistent(mock_sleep, mock_cluster_connection):
    ...
    def mock_inject_chaos(node, chaos_type, **kwargs):
        """Mock chaos injection to track what was injected."""
        chaos_injections.append({"node_id": node.node_id, "chaos_type": chaos_type})
        killed_node_ids.add(node.node_id)
        return ChaosResult(
            chaos_id=f"chaos_{len(chaos_injections)}",
            chaos_type=ChaosType.PROCESS_KILL,
    ...
    chaos_config = ChaosConfig(
        ...
        timing=ChaosTiming(delay_before_operation=0.1, delay_after_operation=0.1),
        coordination=ChaosCoordination(
            chaos_during_operation=True, chaos_before_operation=False, chaos_after_operation=False
        ),
