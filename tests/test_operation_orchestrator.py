...
from src.models import ClusterConnection, NodeInfo, Operation, OperationTiming, OperationType
...
def test_execute_failover_exact_node_id_match():
    ...
    nodes_list = [
        {"node_id": "node-1", "host": "127.0.0.1", "port": 7001, "role": "primary", "shard_id": 0},
        {"node_id": "node-10", "host": "127.0.0.1", "port": 7010, "role": "primary", "shard_id": 1},
        {"node_id": "node-11", "host": "127.0.0.1", "port": 7011, "role": "primary", "shard_id": 2},
    ]
    ...
    operation = Operation(type=OperationType.FAILOVER, target_node="node-1", parameters={}, timing=OperationTiming())
    ...
    with patch("src.utils.valkey_utils.valkey.Valkey") as mock_valkey:
        ...
        # Execute failover
        orchestrator._execute_failover(operation)
...
def test_execute_failover_no_substring_false_positive():
    ...
    nodes_list = [
        {"node_id": "node-10", "host": "127.0.0.1", "port": 7010, "role": "primary", "shard_id": 1},
        {"node_id": "node-11", "host": "127.0.0.1", "port": 7011, "role": "primary", "shard_id": 2},
        {"node_id": "node-12", "host": "127.0.0.1", "port": 7012, "role": "primary", "shard_id": 3},
    ]
    mock_connection.get_current_nodes.return_value = nodes_list
    mock_connection.get_live_nodes.return_value = nodes_list
...
def test_execute_failover_shard_based_matching():
    ...
    mock_connection = Mock()
    nodes_list = [
        {"node_id": "node-0", "host": "127.0.0.1", "port": 6379, "role": "primary", "shard_id": 0},
        {"node_id": "node-30", "host": "127.0.0.1", "port": 6409, "role": "primary", "shard_id": 10},
        {"node_id": "node-31", "host": "127.0.0.1", "port": 6410, "role": "replica", "shard_id": 10},
    ]
    mock_connection.get_current_nodes.return_value = nodes_list
    mock_connection.get_live_nodes.return_value = nodes_list
    ...
    operation = Operation(
        type=OperationType.FAILOVER, target_node="shard-10-primary", parameters={}, timing=OperationTiming()
    )
...
def test_execute_failover_port_matching():
    ...
    mock_connection = Mock()
    nodes_list = [
        {"node_id": "abc123", "host": "127.0.0.1", "port": 7001, "role": "primary", "shard_id": 0},
        {"node_id": "replica123", "host": "127.0.0.1", "port": 7002, "role": "replica", "shard_id": 0},
        {"node_id": "def456", "host": "127.0.0.1", "port": 7010, "role": "primary", "shard_id": 1},
    ]
    mock_connection.get_current_nodes.return_value = nodes_list
    mock_connection.get_live_nodes.return_value = nodes_list
    # Mock find_alive_node to return the replica
    ...
    operation = Operation(type=OperationType.FAILOVER, target_node="7001", parameters={}, timing=OperationTiming())
