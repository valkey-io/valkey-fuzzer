...
from ..utils.valkey_utils import query_cluster_nodes, safe_query_node, valkey_client
        ...
        try:
            ...
            if not replica_nodes:
                ...
                if config.min_replicas_per_shard > 0:
                    ...
                    num_shards = len(
                        set(p.get("shard_id") for p in primary_nodes_list if p.get("shard_id") is not None)
                    )
            ...
            for replica in replica_nodes:
                ...
                try:
                    ...
                    if master_link_status != "up":
                        ...
                        all_replicas_synced = False
                        logger.warning(
                            f"Replica {replica_address} disconnected from primary (link_status={master_link_status})"
                        )
                        continue
                    ...
                    if config.check_replication_offset and master_node_id:
                        replication_offset_diff = self._get_offset_difference(
                            replica, primary_nodes.get(master_node_id), config.timeout
                        )

                    # Check if replica is lagging
                    is_lagging = lag_seconds > config.max_acceptable_lag and (
                        not config.check_replication_offset or replication_offset_diff > 0
                    )
                    if is_lagging:
            ...
            self._cross_validate_with_primaries(primary_nodes, replica_nodes, disconnected_replicas, config)
            ...
            if killed_nodes and disconnected_replicas:
                ...
                unexpected_disconnections = disconnected_set - killed_nodes - replicas_with_expected_link_down
            ...
            if success and config.min_replicas_per_shard > 0:
                redundancy_disconnected_replicas = [
                    replica for replica in disconnected_replicas if replica not in replicas_with_expected_link_down
                ]
            ...
            if success and config.require_all_replicas_synced:
                ...
                if lagging_replicas:
                    ...
                    logger.info(f"{len(disconnected_replicas)} replica(s) disconnected but cluster still functional")
        ...
        for primary_node_id, primary in primary_nodes.items():
            ...
            if repl_info:
                ...
                expected_replicas = sum(
                    1
                    for replica_addr, master_id in replica_to_primary_map.items()
                    if master_id == primary_node_id and replica_addr not in disconnected_replicas
                )
        ...
        try:
            ...
            for node in all_nodes:
                try:
                    ...
                    logger.debug(f"Unable to query cluster info from {node['host']}:{node['port']}: {e}")
            ...
            if cluster_states:
                ...
                if len(set(cluster_states.values())) > 1:
                    logger.warning(f"Cluster state disagreement detected: {cluster_states}")
            else:
            ...
            if overall_cluster_state not in config.acceptable_states:
                ...
                if overall_cluster_state == "unknown" and killed_nodes:
                    ...
                    validation_success, validation_error = validate_killed_vs_failed_nodes(
                        killed_nodes=killed_nodes, failed_nodes=nodes_in_fail_state, context="cluster_status"
                    )
                    ...
                    if not validation_success:
                        ...
                        degraded_reason = (
                            f"Cluster in 'unknown' state due to {len(killed_nodes)} killed node(s) - expected behavior"
                        )
                        logger.info(
                            f"Cluster state 'unknown' is expected: killed nodes {killed_nodes} are in fail state"
                        )
                    ...
                    success = False
                    error_message = (
                        f"Cluster state '{overall_cluster_state}' not in acceptable states: {config.acceptable_states}"
                    )
                    logger.error(error_message)
                ...
                if killed_nodes:
                    ...
                    if success and overall_cluster_state in config.acceptable_states:
                        ...
                        if not config.allow_degraded:
                            ...
                            degraded_reason = (
                                f"{len(nodes_in_fail_state)} node(s) in fail state (degraded state allowed by config)"
                            )
                            logger.warning(f"Nodes in fail state but degraded state is allowed: {nodes_in_fail_state}")
                    else:
            ...
            if not has_quorum:
                success = False
                error_message = (
                    f"Cluster does not have quorum: {len(reachable_primaries)}/{len(primary_nodes)} primaries reachable"
                )
                logger.error(error_message)
    ...
    def validate(
        ...
        try:
            ...
            for slot in range(16384):
                ...
                slot_owners = []
                for _node_addr, slot_view in node_perspectives.items():
            ...
            for slot in range(16384):
                if slot not in slot_assignments:
                    ...
                    conflict = SlotConflict(slot=slot, conflicting_nodes=slot_assignments[slot])
                    conflicting_slots.append(conflict)
                    logger.warning(f"Slot {slot} has conflicting assignments: {slot_assignments[slot]}")
            ...
            if success and config.require_full_coverage and unassigned_slots:
                ...
                unexpected_unassigned = [s for s in unassigned_slots if s not in slots_on_fully_killed_shards]
                if unexpected_unassigned:
                    success = False
                    error_message = (
                        f"{len(unexpected_unassigned)} slots are unassigned (expected all 16384 slots to be assigned)"
                    )
                    logger.error(error_message)
            ...
            if success and not config.allow_slot_conflicts and conflicting_slots:
                success = False
                conflict_msg = f"{len(conflicting_slots)} slots have conflicting assignments"
                error_message = f"{error_message}; {conflict_msg}" if error_message else conflict_msg
                logger.error(error_message)
...
class TopologyValidator:
    ...
    def validate(
        ...
        try:
            ...
            unexpected_failures = detect_unexpected_failures(
                all_nodes=all_nodes, killed_nodes=killed_nodes, context="topology"
            )
            ...
            if not expected_topology:
                logger.info(
                    f"Topology validation (no expectations): {actual_primaries} primaries, {actual_replicas} replicas"
                )
                return TopologyValidation(
                    success=True,
            ...
            promoted_primaries = (
                killed_primaries - fully_killed_primary_count if chaos_killed_shards else killed_primaries
            )
            ...
            if actual_primaries != adjusted_expected_primaries:
                ...
                logger.warning(
                    f"Primary count mismatch: expected {adjusted_expected_primaries}, got {actual_primaries}"
                )
            ...
            if actual_replicas != adjusted_expected_replicas:
                ...
                topology_mismatches.append(mismatch)
                logger.warning(f"Replica count mismatch: expected {adjusted_expected_replicas}, got {actual_replicas}")
            ...
            if expected_topology.shard_structure:
                ...
                shard_mismatches = self._validate_shard_structure(live_nodes, expected_topology.shard_structure, config)
            ...
            if config.strict_mode:
                ...
                if topology_mismatches and success:  # Don't override unexpected failure message
                    success = False
                    error_message = (
                        f"Topology validation failed in strict mode: {len(topology_mismatches)} mismatch(es) found"
                    )
            else:
                ...
                if critical_mismatches:
                    ...
                    mismatch_details = ", ".join([f"{m.mismatch_type}({m.node_id})" for m in critical_mismatches[:5]])
                    ...
                    logger.info(f"Topology validation passed with {len(topology_mismatches)} non-critical mismatch(es)")
    ...
    def _validate_shard_structure(
        self, all_nodes: list[dict], expected_shards: dict[int, "ShardExpectation"], config: "TopologyValidationConfig"
        ...
        for node in all_nodes:
            ...
            if shard_id not in actual_shards:
                actual_shards[shard_id] = {"primary": None, "replicas": []}
        ...
        for shard_id, expected_shard in expected_shards.items():
            ...
            for replica_id in missing_replicas:
                ...
                mismatches.append(mismatch)
                logger.warning(f"Expected replica {replica_id} not found in shard {shard_id}")
            ...
            if config.strict_mode:
                ...
                for replica_id in extra_replicas:
                    ...
                    mismatches.append(mismatch)
                    logger.warning(f"Unexpected replica {replica_id} found in shard {shard_id}")
...
class ViewConsistencyValidator:
        ...
        try:
            ...
            for node in all_nodes:
                ...
                if parsed_nodes:
                    ...
                    logger.debug(f"Collected cluster view from {node_address}: {len(parsed_view)} nodes in view")
            ...
            if killed_nodes:
                ...
                killed_node_ids = set()
                for _node_addr, node_view in node_views.items():
                ...
                for discrepancy in view_discrepancies:
                    ...
                    if is_about_killed_node:
                        ...
                        if discrepancy.discrepancy_type == "membership":
                            ...
                            if (
                                "not in view" in discrepancy.actual_value
                                or "missing" in discrepancy.actual_value.lower()
                            ):
                                is_expected = True
                                logger.debug(
                            ...
                            if (
                                "fail" in discrepancy.actual_value.lower()
                                or "disconnected" in discrepancy.actual_value.lower()
                            ):
                                is_expected = True
                                logger.debug(
    ...
    def _compare_views(self, node_views: dict[str, dict[str, dict]]) -> list["ViewDiscrepancy"]:
        ...
        reference_address = next(iter(node_views.keys()))
        ...
        for node_address, node_view in node_views.items():
            ...
            for missing_node_id in missing_nodes:
                ...
                logger.warning(f"Node {node_address} missing node {missing_node_id} in its view")
            ...
            for extra_node_id in extra_nodes:
                ...
                discrepancies.append(discrepancy)
                logger.warning(f"Node {node_address} has extra node {extra_node_id} in its view")
            ...
            for node_id in common_node_ids:
                ...
                if ref_addr != curr_addr:
                    ...
                    discrepancies.append(discrepancy)
                    logger.warning(
                        f"Node {node_address} sees {node_id} at {curr_addr}, but reference sees it at {ref_addr}"
                    )
    ...
    def _detect_split_brain(self, node_views: dict[str, dict[str, dict]]) -> bool:
        ...
        for _node_address, node_view in node_views.items():
            for node_id, node_info in node_view.items():
        ...
        for slot, primaries in slot_primary_map.items():
            if len(primaries) > 1:
                logger.error(f"Split-brain detected: slot {slot} claimed by multiple primaries: {primaries}")
                return True
    ...
    def _calculate_consensus(self, node_views: dict[str, dict[str, dict]]) -> float:
        ...
        for node_address, node_view in node_views.items():
            ...
            signature = frozenset((node_id, info["role"]) for node_id, info in node_view.items())
...
class DataConsistencyValidator:
    ...
    def write_test_keys(self, cluster_connection: ClusterConnection, config: DataConsistencyValidationConfig) -> bool:
        try:
            ...
            startup_nodes = [ClusterNode(host=node["host"], port=node["port"]) for node in live_nodes[:3]]
    ...
    def validate(
        self, cluster_connection: ClusterConnection, config: DataConsistencyValidationConfig
    ) -> DataConsistencyValidation:
        try:
            ...
            if config.check_cross_replica_consistency:
                self._check_cross_replica_consistency(cluster_connection, config, inconsistent_keys)
            ...
            if inconsistent_keys:
                success = False
                inconsistency_msg = f"Data inconsistency detected: {len(inconsistent_keys)} key(s) have wrong values"
                error_message = f"{error_message}; {inconsistency_msg}" if error_message else inconsistency_msg
                logger.error(error_message)
            ...
            if len(unreachable_keys) >= unreachable_threshold:
                success = False
                unreachable_msg = (
                    f"{len(unreachable_keys)} test key(s) unreachable (threshold: {int(unreachable_threshold)})"
                )
                error_message = f"{error_message}; {unreachable_msg}" if error_message else unreachable_msg
                logger.error(error_message)
    ...
    def _check_cross_replica_consistency(
        ...
        try:
            ...
            logger.debug(
                f"Checking cross-replica consistency for {sample_size} test keys across {len(replica_nodes)} replicas"
            )
                    ...
                    try:
                        with valkey_client(replica["host"], replica["port"], config.timeout) as client:
                            ...
                            if actual_value is None:
                                ...
                                inconsistent_keys.append(inconsistency)
                                logger.warning(f"Key {key} missing on replica {format_node_address(replica)}")
                            elif actual_value != expected_value:
            ...
            if inconsistent_keys:
                logger.warning(f"Cross-replica consistency check found {len(inconsistent_keys)} inconsistencies")
...
class StateValidator:
    ...
    def write_test_data(self, cluster_connection: ClusterConnection) -> bool:
        ...
        return self.data_consistency_validator.write_test_keys(cluster_connection, self.config.data_consistency_config)
    ...
    def _run_validation_check(
        ...
        try:
            ...
            logger.info(f"{display_name} validation: {'PASSED' if result.success else 'FAILED'}")
            ...
            failed_checks.append(check_name)
            error_messages.append(f"{display_name} validation error: {e!s}")
            return None
    ...
    def validate_state(
        ...
        if self.config.stabilization_wait > 0:
            ...
            if actual_wait <= 0:
                ...
                return StateValidationResult(
                    ...
                    error_messages=[
                        f"Validation exceeded timeout of {self.config.validation_timeout}s before stabilization"
                    ],
                )
            ...
            if actual_wait < self.config.stabilization_wait:
                ...
                logger.info(f"Waiting {self.config.stabilization_wait}s for cluster stabilization before validation")
            ...
            if time.time() >= timeout_deadline:
                logger.error(
                    f"Validation timeout ({self.config.validation_timeout}s) exceeded during stabilization wait"
                )
                return StateValidationResult(
                    overall_success=False,
                    validation_timestamp=validation_start,
                    ...
                    failed_checks=["validation_timeout"],
                    error_messages=[
                        f"Validation exceeded timeout of {self.config.validation_timeout}s during stabilization"
                    ],
                )
        ...
        try:
                ...
                if self.config.check_replication:
                    ...
                    replication_result = self._run_validation_check(
                        ...
                        validator_func=lambda: self.replication_validator.validate(
                            cluster_connection, self.config.replication_config, killed_nodes=self.killed_nodes
                        ),
                ...
                if self.config.check_cluster_status:
                    ...
                    cluster_status_result = self._run_validation_check(
                        ...
                        validator_func=lambda: self.cluster_status_validator.validate(
                            cluster_connection, self.config.cluster_status_config, killed_nodes=self.killed_nodes
                        ),
                        failed_checks=failed_checks,
                        error_messages=error_messages,
                ...
                if self.config.check_view_consistency:
                    ...
                    view_consistency_result = self._run_validation_check(
                        ...
                        validator_func=lambda: self.view_consistency_validator.validate(
                            cluster_connection, self.config.view_consistency_config, killed_nodes=self.killed_nodes
                        ),
                        failed_checks=failed_checks,
                        error_messages=error_messages,
                ...
                if self.config.check_data_consistency:
                    ...
                    data_consistency_result = self._run_validation_check(
                        ...
                        validator_func=lambda: self.data_consistency_validator.validate(
                            cluster_connection, self.config.data_consistency_config
                        ),
                        failed_checks=failed_checks,
                        error_messages=error_messages,
                ...
                if self.config.check_logs and operation_context:
                    ...
                    for node_dict in all_nodes:
                        ...
                        matching_node = next(
                            (
                                n
                                for n in cluster_connection.initial_nodes
                    ...
                    if not log_result.success:
                        ...
                        for finding in log_result.findings:
                            if finding.severity == "error":
                                logger.error(
                                    f"Log validation error - Shard {finding.shard_id}, Node {finding.node_id}: {finding.message} | Log: {finding.log_line}"
                                )
                            elif finding.severity == "warning":
                                logger.warning(
                                    f"Log validation warning - Shard {finding.shard_id}, Node {finding.node_id}: {finding.message} | Log: {finding.log_line}"
                                )
            ...
            error_messages.append(f"Unexpected validation error: {e!s}")
        ...
        # Log overall result
        if overall_success:
            logger.info(f"State validation PASSED (duration: {validation_duration:.2f}s)")
        else:
            logger.error(
            ...
            if result.is_critical_failure():
                logger.error("CRITICAL FAILURE DETECTED: Slot coverage lost or split-brain scenario")
    ...
    def validate_with_retry(
        ...
        while attempt < max_attempts:
            ...
            # Execute validation
            result = self.validate_state(cluster_connection, expected_topology, operation_context)

            # If validation passed, return immediately
            if result.overall_success:
                if attempt > 1:
                    logger.info(f"Validation succeeded on attempt {attempt}/{max_attempts}")
                return result

            # Check if this is a critical failure
            if result.is_critical_failure():
                logger.error(
                    f"Critical failure detected - skipping retry. Failed checks: {', '.join(result.failed_checks)}"
                )
                return result
            ...
            if attempt < max_attempts and self.config.retry_on_transient_failure:
                ...
                if not self.config.retry_on_transient_failure:
                    logger.error(
                        f"Validation failed and retry is disabled. Failed checks: {', '.join(result.failed_checks)}"
                    )
                else:
                    logger.error(
