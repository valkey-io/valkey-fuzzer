...
class ShardLogValidator:
    ...
    def __init__(self):
        ...
        self.replication_issue_patterns = [r"I'm a sub-replica! Reconfiguring myself"]

    def validate_affected_shards(
        self,
        cluster_nodes: list[NodeInfo],
        operations: list[Operation],
        nodes_killed_by_chaos: set[str],
        shards_with_primary_killed: set[int],
    ) -> LogValidationResult:
    ...
    def _get_affected_shards(
        self, operations: list[Operation], nodes_killed_by_chaos: set[str], cluster_nodes: list[NodeInfo]
    ) -> set[int]:
        ...
        for op in operations:
            ...
            if "shard-" in op.target_node:
                ...
                shard_id = (
                    node_id_to_shard.get(op.target_node)
                    or port_to_shard.get(op.target_node)
                    or cluster_id_to_shard.get(op.target_node)
                )
    ...
    def _validate_failure_detection(
        self, cluster_nodes: list[NodeInfo], nodes_killed_by_chaos: set[str]
    ) -> list[LogFinding]:
        ...
        for node in affected_nodes:
            ...
            try:
                ...
                if node.role == "primary":
                    has_promotion = any(
                        self._has_pattern(log_lines, pattern) for pattern in self.failover_promotion_patterns
                    )
                        ...
                        if matches:
                            findings.append(
                                LogFinding(
                                    ...
                                    shard_id=node.shard_id,
                                    severity="error",
                                    message="Failover error detected",
                                    log_line=matches[0],
                                )
                            )
                    ...
                    if matches:
                        findings.append(
                            LogFinding(
                                ...
                                shard_id=node.shard_id,
                                severity="warning",
                                message="Replication topology issue detected",
                                log_line=matches[0],
                            )
                        )
