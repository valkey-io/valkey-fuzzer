...
from ..models import ChaosResult, ClusterStatus, ExecutionResult, Operation, Scenario
...
class FuzzerLogger:
    ...
    def log_test_start(self, scenario: Scenario) -> None:
        ...
        with self._lock:
            ...
            self.test_logs[scenario.scenario_id] = {
                ...
                "state_validation_config": self._serialize_state_validation_config(scenario.state_validation_config)
                if scenario.state_validation_config
                else None,
                "operation_logs": [],
                "chaos_events": [],
    ...
    def log_operation(self, operation: Operation, success: bool, details: str, silent: bool = False) -> None:
        ...
        with self._lock:
            ...
            if not silent:
                logger.info(
                    f"Logged operation: {operation.type.value} on {operation.target_node} - {'SUCCESS' if success else 'FAILED'}"
                )
    ...
    def log_chaos_event(self, chaos_result: ChaosResult) -> None:
        ...
        with self._lock:
            ...
            chaos_log = {
                ...
                "end_timestamp": datetime.fromtimestamp(chaos_result.end_time).isoformat()
                if chaos_result.end_time
                else None,
            ...
            logger.info(
                f"Logged chaos event: {chaos_result.chaos_type.value} on {chaos_result.target_node} - {'SUCCESS' if chaos_result.success else 'FAILED'}"
            )
            self._write_log_to_disk(self.current_test_id)
    ...
    def log_state_validation_result(self, validation_result, operation_number: int) -> None:
        ...
        if validation_result.slot_coverage:
            ...
            validation_log["checks"]["slot_coverage"] = {
                ...
                "unassigned_slots": slots.unassigned_slots[:100]
                if len(slots.unassigned_slots) > 100
                else slots.unassigned_slots,  # Limit for readability
                "conflicting_slots": [
                    {"slot": conflict.slot, "conflicting_nodes": conflict.conflicting_nodes}
                    for conflict in slots.conflicting_slots
                ],
                "slot_distribution": {
                    node_id: len(slot_list) for node_id, slot_list in slots.slot_distribution.items()
                },
    ...
    def log_test_completion(self, test_result: ExecutionResult) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Completed test {test_result.scenario_id} - {'SUCCESS' if test_result.success else 'FAILED'} "
    ...
    def _log_scenario_summary(self, scenario: Scenario) -> None:
        ...
        for i, op in enumerate(scenario.operations, 1):
            ...
            summary_lines.append(
                f"     Timing: delay_before={op.timing.delay_before:.2f}s, timeout={op.timing.timeout:.2f}s, delay_after={op.timing.delay_after:.2f}s"
            )
