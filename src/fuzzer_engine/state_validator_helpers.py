...
def validate_killed_vs_failed_nodes(
    killed_nodes: set[str], failed_nodes: list[str], context: str = "validation"
...
def check_per_shard_redundancy(
    ...
    if shards_without_redundancy:
        shard_details = ", ".join(
            [
                f"shard {s['shard_id']} ({s['alive_replicas']} alive, {s['dead_unexpected']} unexpected failures)"
...
def detect_unexpected_failures(all_nodes: list[dict], killed_nodes: set[str], context: str = "topology") -> list[dict]:
    ...
    for node in all_nodes:
        if node.get("status") == "failed" and not is_node_killed_by_chaos(node, killed_nodes):
