...
from .error_handler import ErrorCategory, ErrorContext, ErrorHandler, ErrorSeverity, RetryConfig
...
class FuzzerEngine(IFuzzerEngine):
    ...
    def execute_test(self, scenario: Scenario) -> ExecutionResult:
        ...
        try:
            ...
            cluster_connection = ClusterConnection(
                initial_nodes=cluster_instance.nodes, cluster_id=cluster_instance.cluster_id
            )
            ...
            if expected_replicas_per_shard == 0:
                ...
                logger.info(
                    f"Adjusted min_replicas_per_shard to {expected_replicas_per_shard} to match cluster configuration"
                )
            ...
            state_validator.set_nodes_per_shard(
                {shard_id: nodes_per_shard_count for shard_id in range(scenario.cluster_config.num_shards)}
            )
            ...
            def validate_operation_wave(wave_number, wave_results):
                ...
                self._register_killed_nodes(state_validator, cluster_instance.nodes, wave_chaos_events)
                ...
                validation_result = state_validator.validate_with_retry(
                    cluster_connection, expected_topology, operation_context
                )
            ...
            for op in scenario.operations:
                for log in operation_logs:
                    if (
                        log.get("success")
                        and log.get("operation_type") == op.type.value
                        and log.get("target_node") == op.target_node
                    ):
    ...
    def validate_cluster_state(self, cluster_id: str):
        ...
        replicas_per_shard = actual_replicas // actual_primaries if actual_primaries > 0 else 0
        ...
        if replicas_per_shard == 0:
            ...
            logger.info(f"Adjusted min_replicas_per_shard to {replicas_per_shard} to match actual cluster topology")

        validator = StateValidator(validation_config)

        # Build expected topology
        expected_topology = ExpectedTopology(
            num_primaries=actual_primaries, num_replicas=actual_replicas, shard_structure={}
        )
    ...
    def _register_killed_nodes(self, state_validator: StateValidator, cluster_nodes, chaos_events: list) -> None:
    ...
    def _collect_failure_reasons(
        ...
        if chaos_config:
            has_interfering_chaos = any(
                isinstance(event, ChaosResult) and event.success and event.chaos_phase in ("before", "during")
            ...
            if not has_interfering_chaos and chaos_config.randomize_per_operation:
                has_interfering_chaos = any(isinstance(event, ChaosResult) and event.success for event in chaos_events)
        ...
        if validation_results:
            failed_validation_waves = [result for result in validation_results if not result.overall_success]
            if failed_validation_waves:
                failure_reasons.append(f"{len(failed_validation_waves)} post-operation validation wave(s) failed")
    ...
    def _create_cluster_with_retry(self, scenario: Scenario, max_retries: int = 3):
        ...
        retry_config = RetryConfig(max_attempts=max_retries, initial_delay=1.0, exponential_base=2.0, jitter=True)
    ...
    def _validate_cluster_readiness_with_retry(self, cluster_id: str, max_retries: int = 5) -> bool:
        ...
        retry_config = RetryConfig(max_attempts=max_retries, initial_delay=2.0, exponential_base=1.5, jitter=False)
        ...
        if not success:
            self.logger.log_error(
                "Cluster readiness validation failed", {"attempts": max_retries, "cluster_id": cluster_id}
            )
