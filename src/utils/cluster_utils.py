...
def find_node_by_identifier(
    nodes: list[dict], identifier: str, match_strategies: Optional[list[str]] = None
) -> Optional[dict]:
    if not nodes:
        return None
    ...
    if "shard_pattern" in match_strategies and "shard-" in identifier:
        try:
            ...
            if len(parts) >= 3 and parts[0] == "shard":
                ...
                # Map role names
                role_map = {"primary": "primary", "master": "primary", "replica": "replica", "slave": "replica"}
...
def find_primary_node_by_identifier(
    ...
    if node:
        logger.warning(f"Found node matching '{identifier}' but it's not a primary (role: {node.get('role')})")

    return None


def find_replica_nodes_by_primary(
    nodes: list[dict], primary_node_id: str, primary_shard_id: Optional[int] = None
) -> list[dict]:
    replicas = []
    ...
    if primary_shard_id is not None:
        for node in nodes:
            if node.get("role") == "replica" and node.get("shard_id") == primary_shard_id:
                ...
                logger.debug(f"Found replica by shard_id: {node.get('node_id')} at port {node.get('port')}")
    ...
    if not replicas:
        for node in nodes:
            if node.get("role") == "replica" and node.get("master_id") == primary_node_id:
                replicas.append(node)
                logger.debug(f"Found replica by master_id: {node.get('node_id')} at port {node.get('port')}")
